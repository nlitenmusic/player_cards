    ...
    <div style={{ display: 'flex', justifyContent: 'center', paddingTop: 20 }}>
      <div style={{ width: 820, maxWidth: '100%', background: '#fff', padding: 16, borderRadius: 12 }}>
        ...
        <div style={{ marginTop: 12, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 12 }}>
          {rows.map((r) => (
            <div key={r.skill_type} style={{ borderRadius: 10, padding: 12, background: '#F8FAFC', minHeight: 120, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                ...
                {(['c','p','a','s','t'] as const).map((ck) => {
                  ...
                  return (
                    <div key={ck} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, width: '100%', maxWidth: 220 }}>
                      <div style={{ color: '#374151', width: 110, textAlign: 'left' }}>{(ck === 'c' && 'Consistency') || (ck === 'p' && 'Power') || (ck === 'a' && 'Accuracy') || (ck === 's' && 'Spin') || (ck === 't' && 'Technique')}</div>
